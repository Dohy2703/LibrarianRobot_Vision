...
import time
from ..loss import DiceLoss, EmbLoss_v1, ohem_batch, iou
from ..utils.nas_utils import set_layer_from_config
from ..utils.generate_bbox import generate_bbox
...
class FASTHead(nn.Module):
    def __init__(self, conv, blocks, final, 
                pooling_size=7, dropout_ratio=0.1):
        ...
        self.text_loss = DiceLoss(loss_weight=0.5)
        self.kernel_loss = DiceLoss(loss_weight=1.0)
...
def fast_head():
    head_config = json.load(open(os.path.join(Path(__file__).parent, 'head.conf'), 'r'))
    head = FASTHead.build_from_config(head_config)
    return head